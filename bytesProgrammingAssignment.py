...
for file in glob.glob(path):        #Iterate through all the files in the logs folder
    ...
    sorted_dict = dict(sorted(  freq.items(), key=operator.itemgetter(1), reverse=True))        #Sort the dictionary
    for key, value in sorted_dict.items():     #Display all the items in the freq dictionary
